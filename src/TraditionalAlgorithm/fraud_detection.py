...
def preprocess(credit: pd.DataFrame) -> pd.DataFrame:
    ...
    credit.loc[credit['work_year'] > 0,
               'work_year'] = credit[credit['work_year'] < 0]['work_year'].max()
    ...
    # 丢弃部分列
    credit = credit.drop(
        columns=['FLAG_EMAIL', 'FLAG_WORK_PHONE', 'housing-status'])
    return credit
...
if __name__ == '__main__':
    ...
    X_train, X_test, Y_train, Y_test = train_test_split(
        X, Y, train_size=0.7, random_state=10086)
    # 平衡数据集
